...
-- -----------------------------------------------------
-- Table `inventory`.`addressingsstocktaking`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `inventory`.`addressingsstocktaking` ;

CREATE TABLE IF NOT EXISTS `inventory`.`addressingsstocktaking` (
  `Id` INT NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`Id`),
  INDEX `IX_AddressingsStockTaking_AddressingId` (`AddressingId` ASC) VISIBLE,
  INDEX `IX_AddressingsStockTaking_InventoryStartId` (`InventoryStartId` ASC) VISIBLE,
  CONSTRAINT `FK_AddressingsStockTaking_Addressing_AddressingId`
    FOREIGN KEY (`AddressingId`)
    REFERENCES `inventory`.`addressing` (`Id`)
    ON DELETE CASCADE,
  CONSTRAINT `FK_AddressingsStockTaking_InventoryStart_InventoryStartId`
...
CREATE TABLE IF NOT EXISTS `inventory`.`stocktaking` (
  ...
  PRIMARY KEY (`Id`),
  INDEX `IX_StockTaking_AddressingId` (`AddressingId` ASC) VISIBLE,
  INDEX `IX_StockTaking_InventoryStartId` (`InventoryStartId` ASC) VISIBLE,
  INDEX `IX_StockTaking_ItemId` (`ItemId` ASC) VISIBLE,
  CONSTRAINT `FK_StockTaking_Addressing_AddressingId`
    FOREIGN KEY (`AddressingId`)
    REFERENCES `inventory`.`addressing` (`Id`)
    ON DELETE CASCADE,
  CONSTRAINT `FK_StockTaking_InventoryStart_InventoryStartId`
    FOREIGN KEY (`InventoryStartId`)
    REFERENCES `inventory`.`inventorystart` (`Id`)
    ON DELETE CASCADE,
