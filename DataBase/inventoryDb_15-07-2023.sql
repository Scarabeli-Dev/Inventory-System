...
INSERT INTO `__efmigrationshistory` VALUES ('20230712210127_firstMigration','6.0.18');
...
INSERT INTO `aspnetroles` VALUES (1,'Admin','ADMIN','534366c8-2bb4-46b2-b458-2e759ebb94de'),(2,'Gerente','GERENTE','f115d4d6-c8ec-490f-ba9e-4c2042dc6259'),(3,'Contagem','CONTAGEM','a4cdf2a8-c1dd-4f70-b9a5-2313fd32afac'),(4,'Relatorio','RELATORIO','46b25c47-9af1-48e7-bf34-c565d110ca80');
...
INSERT INTO `aspnetusers` VALUES (1,'Elanã',0,'esesena','ESESENA','elana.scarabeli@rcproconsultoria.com.br','ELANA.SCARABELI@RCPROCONSULTORIA.COM.BR',1,'AQAAAAEAACcQAAAAEKCmFPKBgTMOS0RJ7e2KorEh6SeGayzTY9b028+dAP9Kb6fOHYw4/RkwVWneqYzv2Q==','I5XZGKVV3ERAEKBEHTMHIDD2YYCBLRWT','6b6c0c94-5ea4-48c7-a74f-d51d7cdc6489',NULL,0,0,NULL,1,0);
...
CREATE TABLE `inventorystart` (
  ...
  PRIMARY KEY (`Id`),
  KEY `IX_InventoryStart_WarehouseId` (`WarehouseId`),
  CONSTRAINT `FK_InventoryStart_Warehouse_WarehouseId` FOREIGN KEY (`WarehouseId`) REFERENCES `warehouse` (`Id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `perishableitem` (
  ...
  `PerishableItemPreviousQuantity` decimal(18,2) NOT NULL,
...
CREATE TABLE `stocktaking` (
  ...
  `StockTakingPreviousQuantity` decimal(18,2) NOT NULL,
  ...
  `ItemToRecount` tinyint(1) NOT NULL,
...
/*!50001 VIEW `viewstocktakingfinalreport` AS select distinct `i`.`Id` AS `ItemId`,`i`.`Name` AS `ItemName`,`i`.`UnitOfMeasurement` AS `UnitOfMeasurement`,coalesce(`ia`.`Quantity`,0) AS `SystemQuantity`,coalesce(`st`.`StockTakingQuantity`,0) AS `QuantityStockTaking`,`ia`.`Id` AS `ItemsAddressingId`,coalesce(`ad`.`WarehouseId`,0) AS `WarehouseAddressingId`,coalesce(`st`.`AddressingsInventoryStartId`,`ais`.`AddressingId`,0) AS `WarehouseStocktakingId`,(coalesce(`ia`.`Quantity`,0) - coalesce(`st`.`StockTakingQuantity`,0)) AS `Divergence`,1 AS `StockSituation`,1 AS `AddressingSituation` from (((((`item` `i` left join `itemsaddressing` `ia` on((`i`.`Id` = `ia`.`ItemId`))) left join (select `stocktaking`.`Id` AS `Id`,`stocktaking`.`StockTakingDate` AS `StockTakingDate`,`stocktaking`.`ItemId` AS `ItemId`,`stocktaking`.`AddressingsInventoryStartId` AS `AddressingsInventoryStartId`,`stocktaking`.`StockTakingQuantity` AS `StockTakingQuantity`,`stocktaking`.`StockTakingPreviousQuantity` AS `StockTakingPreviousQuantity`,`stocktaking`.`IsPerishableItem` AS `IsPerishableItem`,`stocktaking`.`StockTakingObservation` AS `StockTakingObservation`,`stocktaking`.`ItemToRecount` AS `ItemToRecount`,`stocktaking`.`NumberOfCount` AS `NumberOfCount` from `stocktaking` where `stocktaking`.`Id` in (select min(`stocktaking`.`Id`) from `stocktaking` group by `stocktaking`.`ItemId`)) `st` on((`i`.`Id` = `st`.`ItemId`))) left join `inventorymovement` `im` on((`i`.`Id` = `im`.`ItemId`))) left join `addressing` `ad` on((`ia`.`AddressingId` = `ad`.`Id`))) left join `addressingsinventorystart` `ais` on((`st`.`AddressingsInventoryStartId` = `ais`.`Id`))) order by `i`.`Id` */;
...
-- Dump completed on 2023-07-15 11:13:40
